...
// 1. SVG clock face
clockFace = document.querySelector('svg g.clock--face');
...
entries.forEach(([key, value]) => {
  ...
  // span = document.querySelector(`span.control--${key}`);
...
function updateValues(instructions) {
  ...
  */
  var { key, operation } = instructions;
  var { timeValue, rotationValue } = instructions;

  // create a number of degrees based on the previous value and the current operation
  degrees = operation === '+' ? rotationValue + 1 : rotationValue - 1;
...
function handleClick(key) {
  ...
  //key=hours|minutes|hours
  //key = this.parentElement.getAttribute('data-control');
  ...
  var { value, degrees } = updateValues(instructions);
